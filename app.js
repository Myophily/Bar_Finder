...
function loadKakaoMapScript() {
  ...
  script.onerror = function () {
    alert(
      "Kakao Map API를 로드하는데 실패했습니다. config.js에서 API 키를 확인해주세요."
    );
  };
...
function onRegionDrawn(data) {
  ...
  document.getElementById(
    "regionText"
  ).textContent = `면적: ${areaKm2} km² | 중심: (${bounds.center.lat.toFixed(
    4
  )}, ${bounds.center.lng.toFixed(4)})`;
  document.getElementById("gridInfo").style.display = "block";
  document.getElementById(
    "gridText"
  ).textContent = `${App.gridCells.length}개의 100m×100m 그리드 생성됨`;
...
function calculateArea(bounds) {
  ...
  const lngMeters =
    lngDiff * 111000 * Math.cos((bounds.center.lat * Math.PI) / 180);

  return Math.abs(latMeters * lngMeters);
}

// Generate 100m x 100m grid cells
function generateGrid(bounds) {
  ...
  const lngDegreeMeters =
    111000 * Math.cos((bounds.center.lat * Math.PI) / 180);
...
function downloadCSV() {
  ...
  link.setAttribute(
    "download",
    `bar_search_results_${new Date().toISOString().split("T")[0]}.csv`
  );
