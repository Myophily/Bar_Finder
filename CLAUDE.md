...
This is a **client-side web application** that uses the Kakao Map API to search for bars and restaurants within user-defined regions. The app overcomes Kakao API pagination limits (max 675 results per query) by automatically dividing large search areas into 100m×100m grid cells and aggregating results.
...
2. **Region Selection** (`initializeDrawingManager` → `onRegionDrawn`)
   ...
   - Generates 100m×100m grid cells covering the entire region
...
3. **Grid-Based Search** (`startSearch` → `searchGridCell`)
   ...
   - For each cell: searches from center point with 71m radius (covers entire 100m×100m cell)
...
**Grid Cell Size (100m×100m)**:

- Optimized for maximum coverage in extremely dense areas
- Search radius of 71m covers entire cell (diagonal/2 = √(100²+100²)/2 ≈ 71m)
- 100 cells per km² = comprehensive coverage but significantly more API calls
...
- **GRID_CELL_SIZE**: 100 (meters) - Controls search granularity
- **SEARCH_RADIUS**: 71 (meters) - Must cover grid cell diagonal
...
2. **Dense Area Coverage**: Maximum 675 results per grid cell
   ...
   - With 100m×100m cells, it's extremely unlikely to exceed 675 bars per cell
   - Current grid size is already highly optimized for dense areas
